...
returns `true`. The predicate `fun` is passed `DataFrameRow`s.

If `view=false` a freshly allocated `DataFrame` is returned.
If `view=true` then a `SubDataFrame` view into `df` is returned.

See also: [`filter!`](@ref), [`subset`](@ref), [`subset!`](@ref)
...
The predicate `fun` is passed `DataFrameRow`s.
...
# make sure we disable potential specializations based on eltype for safety reasons
Base.IteratorEltype(::Type{<:AbstractDataFrame}) = Base.EltypeUnknown()

# we do not define length for DataFrame
Base.IteratorSize(::Type{<:AbstractDataFrame}) = Base.SizeUnknown()
