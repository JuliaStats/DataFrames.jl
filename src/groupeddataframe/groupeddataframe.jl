...
Base.IteratorEltype(::Type{<:GroupedDataFrame}) = Base.EltypeUnknown()
...
The predicate `fun` is called with a `SubDataFrame` for each group.
...
```
"""
Base.filter(f, gdf::GroupedDataFrame) = gdf[[f(sdf)::Bool for sdf in gdf]]
